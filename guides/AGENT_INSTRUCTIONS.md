...
### Rules
- **Colors come from Tyrell, layout from Tailwind.** Use `ty-bg-primary` / `ty-text++` / `ty-elevated`, never `bg-blue-500` / `text-gray-900`. To rebrand the whole library, load `tyrell-theme.css` after `tyrell.css` and set `--ty-primary-seed: oklch(0.5 <chroma> <hue>)` on `:root` — every component retints coherently in light + dark mode (formula: `oklch(L-curve[shade] × flavor-l-factor, flavor-chroma × sat-curve[shade], flavor-hue)`; the seed's lightness is discarded, only hue + chroma are read). **Seed the flavor — do not set `--ty-primary-hue` / `--ty-primary-chroma` on `:root` alone.** Dark mode re-declares those dials at `html.dark`, which outranks `:root`, so a dials-only override applies in light and silently reverts to the stock palette in dark. Nothing re-declares `--ty-*-seed`. If you do drive the dials, repeat them inside `html.dark` as well.
- **Flavors are open-ended.** Built-ins: `primary` / `success` / `danger` / `warning` / `neutral` (+`+`/`-` shade suffixes). For a project-specific flavor, don't hardcode colors — define `--ty-color-X` / `--ty-bg-X` / `--ty-solid-X` tokens once and use `flavor="X"` on `ty-button` / `ty-tag`; missing tokens fall back to neutral.
...
- **Composition rules.** `ty-select` takes `<ty-option>` children (not native `<option>`); add `multiple` for multi-select and pair with `<ty-selected-options>` for chips. `ty-tabs`/`ty-wizard` take `<ty-tab>`/`<ty-step>`; `ty-tabs` scrolls by default (72px min per tab via `--ty-tab-min-width`, `…` menu on overflow) — add `fixed` for **2–5** short labels that should split the bar equally (past ~5 the shares are too narrow and labels ellipsize — keep the scrollable default). `ty-radio-group` takes `<ty-radio>`.

### Components available
ty-button, ty-input, ty-textarea, ty-checkbox, ty-switch, ty-radio-group / ty-radio,
ty-select / ty-option / ty-selected-options, ty-copy, ty-file-upload,
