(ns app.lazy.uix-counter
  (:require [shadow.lazy :as lazy]
            [tyrell.shim :as shim]))

(def config (lazy/loadable app.components.uix-counter/configuration))
  ...
  {:observed  [:initial]
   :connected (fn [^js el]
                (let [shadow (shim/ensure-shadow el)
                      t0     (js/Date.now)]
                  (set! (.-innerHTML shadow) (loading-html "uix"))
                  (-> (lazy/load config)
                      (.then (fn [cfg]
                               (set! (.-innerHTML shadow) "")
                               (notify-loaded! el "app-uix" (- (js/Date.now) t0))
                               ((:connected cfg) el))))))
   :attr      (fn [^js el delta]
                (-> (lazy/load config)
