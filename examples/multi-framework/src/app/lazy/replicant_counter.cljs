(ns app.lazy.replicant-counter
  (:require [shadow.lazy :as lazy]
            [tyrell.shim :as shim]))

(def config (lazy/loadable app.components.replicant-counter/configuration))
  ...
  {:observed  [:initial]
   :connected (fn [^js el]
                (let [shadow (shim/ensure-shadow el)
                      t0     (js/Date.now)]
                  (set! (.-innerHTML shadow) (loading-html "replicant"))
                  (-> (lazy/load config)
                      (.then (fn [cfg]
                               (set! (.-innerHTML shadow) "")
                               (notify-loaded! el "app-replicant" (- (js/Date.now) t0))
                               ((:connected cfg) el))))))
   :attr      (fn [^js el delta]
                (-> (lazy/load config)
